...
def file_name(prefix, is_intermediate, size, lw, mu, rec, ejw, off, PP, TBS, cut):
    lw_flag = "1" if lw else "0"
    rec_flag = "1" if rec else "0"
    ejw_str = "0" if (ejw < 0) else str(int(ejw * 1000))
    div = "/temp_" if is_intermediate else "/"
    return prefix + div + str(size) + "s_" + str(mu) + "mu_" + lw_flag + "lw_" + rec_flag + "rec_" + ejw_str + "ejw_" + str(off) + "off_" + str(PP) + "PP_" + str(TBS) + "TBS_" + str(cut) + "cut"

#def subprocess_call_bg(args):
#    subprocess.call(args)

def submit_fuzzy(mu, size, lw, rec, ejw, off, PP, TBS, cut, i, unique_id):
    ...
    lw_flag = "1" if lw else "0"
    rec_flag = "1" if rec else "0"
    outfile_name = file_name(scratchdir, True, size, lw, mu, rec, ejw, off, PP, TBS, cut) + "_" + str(i) + ".root"
...
event_jet_offsets = [0]
seed_cuts = [5]#[0, 0.2, 1.0] # should be decimal

j = 0
cleanup_commands = []

for current_cut in seed_cuts:
                        ...
                        for current_rec in recombinations:
                            for current_ejw in event_jet_weights:
                                for current_offset in event_jet_offsets:
                                    out_base = file_name(outdir, False, current_size, current_lw, current_mu, current_rec, current_ejw, current_offset, current_PP, current_TBS, current_cut)
                                    cleanup_base = file_name(outdir, True, current_size, current_lw, current_mu, current_rec, current_ejw, current_offset, current_PP, current_TBS, current_cut)
                                    ...
                                    for current_job in range(n_jobs):
                                        submit_fuzzy(current_mu, current_size, current_lw, current_rec, current_ejw, current_offset, current_PP, current_TBS, current_cut, current_job, j)
                                        j += 1
...
print "SUBMITTED " + str(len(seed_cuts) * len(TBSs) * len(PPs) * len(NPVs) * len(learnWeights) * len(sizes) * (n_jobs + n_jobs_patch) * len(recombinations) * len(event_jet_weights) * len(event_jet_offsets)) + " JOBS TO THE QUEUE " + queue
